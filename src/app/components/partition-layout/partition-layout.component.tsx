...
export default function PartitionLayout (props: IPartitionLayoutProps) {
  const { className, aggregations, handleAggregationChange } = props
  ...
  return (
    <section className={componentClass}>
      <svg
        ...
        ref={(domNode) => renderLayout(domNode, aggregations, handleAggregationChange)}
      >
        <defs>
 ...
 */
function renderLayout (domNode: SVGSVGElement, aggregations: IAggregation, handleAggregationChange: (order: string[]) => void) {
  setTimeout(() => {
    window.requestAnimationFrame(() => {
      renderD3PartitionLayout({ domNode, aggregations, handleAggregationChange })
    })
  }, 0)
}
